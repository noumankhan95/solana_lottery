...
pub mod lottery {
    ...
    pub fn initialize_config(
        ...
        *ctx.accounts.lottery_account = TokenLottery {
        ...
        create_metadata_accounts_v3(
            CpiContext::new_with_signer(
                ...
                CreateMetadataAccountsV3 {
                    ...
                    mint: ctx.accounts.ticket_mint.to_account_info(),
                    mint_authority: ctx.accounts.ticket_mint.to_account_info(),
                    payer: ctx.accounts.payer.to_account_info(),
                    rent: ctx.accounts.rent.to_account_info(),
                    system_program: ctx.accounts.system_program.to_account_info(),
                    update_authority: ctx.accounts.payer.to_account_info(),
    ...
    pub fn choose_a_winner(ctx: Context<ChooseWinner>) -> Result<()> {
        ...
        require!(
            token_lottery.winner_claimed == false,
...
pub struct BuyTicket<'info> {
    ...
    #[account(init,payer=payer,seeds=[token_lottery.total_tickets.to_le_bytes().as_ref()],bump,mint::decimals=0,mint::authority=collection_mint_account,mint::freeze_authority=collection_mint_account,mint::token_program=token_program)]
    pub ticket_mint: InterfaceAccount<'info, Mint>,
...
pub struct Initialize<'info> {
    ...
    #[account(init,payer=signer,seeds=[b"lottery"],bump,space=8+TokenLottery::INIT_SPACE)]
    pub lottery_account: Account<'info, TokenLottery>,
    pub system_program: Program<'info, System>,
}
